...
// ─── Algoritmo de Fracionamento (Bin Packing) ────────────────────────
function agruparPedidosEmLotes(pedidos: Pedido[]): Lote[] {
  if (pedidos.length === 0) return [];

  const lotes: Lote[] = [];
  let loteAtual: Pedido[] = [];
  let itensLoteAtual = 0;
  let valorLoteAtual = 0;

  for (const pedido of pedidos) {
    const itensPedido = (pedido.itens || []).reduce(
      (sum, item) => sum + (item.quantidade || 0), 0,
    );

    // Se adicionar este pedido ultrapassar a capacidade, fecha o lote atual
    if (loteAtual.length > 0 && itensLoteAtual + itensPedido > CAPACIDADE_MAXIMA) {
      lotes.push({
        numero: lotes.length + 1,
        pedidos: loteAtual,
        totalItens: itensLoteAtual,
        valorTotal: valorLoteAtual,
      });
      loteAtual = [];
      itensLoteAtual = 0;
      valorLoteAtual = 0;
    ...
    loteAtual.push(pedido);
    itensLoteAtual += itensPedido;
    valorLoteAtual += pedido.total || 0;
  }

  // Último lote (sempre haverá pelo menos um pedido remanescente)
...
function RotasDeEntrega() {
  ...
  const [loteLiberandoIdx, setLoteLiberandoIdx] = useState<number | null>(null);
  const [loteLiberadoIdx, setLoteLiberadoIdx] = useState<number | null>(null);
  ...
  // ─── Algoritmo de fracionamento via useMemo ────────────────────────
  const lotes = useMemo(() => agruparPedidosEmLotes(pedidosProntos), [pedidosProntos]);
  ...
  // ─── Liberar lote específico ───────────────────────────────────────
  const handleLiberarLote = useCallback(async (lote: Lote) => {
    setLoteLiberandoIdx(lote.numero);
    setErro(null);

    try {
      const ids = lote.pedidos.map((p) => p.id);
      await liberarLote(ids);
      setLoteLiberadoIdx(lote.numero);

      setTimeout(() => {
        setLoteLiberadoIdx(null);
        carregarLote();
        carregarEmTransito();
      }, 1500);
    } catch (err: any) {
      setErro(`Erro ao liberar o Lote #${lote.numero}: ${err.message || String(err)}`);
    } finally {
      setLoteLiberandoIdx(null);
    }
  }, [carregarLote, carregarEmTransito]);
    ...
    <div className="animate-fade-in">
      ...
      {loteLiberadoIdx !== null && (
        <div className="mb-4 animate-slide-up rounded-xl border border-sucesso/20 bg-sucesso/10 px-4 py-3 text-sm font-semibold text-sucesso">
          ✅ Lote #{loteLiberadoIdx} liberado com sucesso! Pedidos enviados para entrega.
        </div>
      )}
      ...
      {!carregando && (
        <>
          ...
          {lotes.length === 0 ? (
              ...
              <div className="p-6 sm:p-8">
                <div className="mb-6 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-grafite-400">Nenhum pedido pronto no momento</p>
                  </div>
                </div>
            ...
            lotes.map((lote) => {
              const porcentagem = Math.min((lote.totalItens / CAPACIDADE_MAXIMA) * 100, 100);
              // Lote liberável se >= 900 itens OU se for o único lote existente
              const loteDisponivel = lote.totalItens >= VOLUME_MINIMO || lotes.length === 1;
              const estaLiberando = loteLiberandoIdx === lote.numero;
              ...
              return (
                <div
                  key={lote.numero}
                  className="mb-8 overflow-hidden rounded-2xl border border-grafite-100 bg-white shadow-soft"
                >
                  ...
                  <div className="p-6 sm:p-8">
                    ...
                    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h2 className="text-xl font-bold text-grafite-800">Lote #{lote.numero}</h2>
                          <p className="text-sm text-grafite-400">
                            {lote.pedidos.length} pedido{lote.pedidos.length !== 1 ? 's' : ''} pronto{lote.pedidos.length !== 1 ? 's' : ''} para sair
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="mb-2">
                      <div className="mb-1.5 flex items-center justify-between text-xs font-medium text-grafite-500">
                        ...
                        <span className="font-bold">{lote.totalItens} / {CAPACIDADE_MAXIMA} itens</span>
                      </div>
                    ...
                    <div className="mt-6 flex justify-center">
                      <div className="relative">
                        ...
                        <button
                          ...
                          onClick={() => handleLiberarLote(lote)}
                          disabled={!loteDisponivel || estaLiberando}
                          title={
                            loteDisponivel
                              ? `Clique para liberar o Lote #${lote.numero} para entrega`
                              : `Aguardando volume mínimo (${VOLUME_MINIMO} itens) — faltam ${VOLUME_MINIMO - lote.totalItens} itens`
                          }
                        >
                          {estaLiberando ? (
                            <>
                              <span className="inline-block h-5 w-5 animate-spin rounded-full border-2 border-white/30 border-t-white" />
                              Liberando Lote #{lote.numero}...
                            </>
                          ) : (
                            <>🚚 Liberar Lote #{lote.numero}</>
                          )}
                        </button>
                      </div>
                    </div>

                    {!loteDisponivel && lote.totalItens > 0 && (
                      <p className="mt-3 text-center text-xs font-medium text-aviso">
                        Faltam {VOLUME_MINIMO - lote.totalItens} itens para atingir o volume mínimo de {VOLUME_MINIMO} unidades.
                      </p>
                    )}

                    {/* ─── Lista de pedidos do lote ─── */}
                    <div className="mt-6 rounded-xl border border-grafite-100">
                      <div className="border-b border-grafite-100 px-5 py-3">
                        <h3 className="text-sm font-bold text-grafite-700">
                          Pedidos no Lote #{lote.numero} ({lote.pedidos.length})
                        </h3>
                      </div>
